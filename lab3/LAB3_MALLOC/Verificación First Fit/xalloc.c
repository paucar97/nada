	...
	for (p= prevp->s.ptr; ; prevp = p, p = p->s.ptr) {
		if (p->s.size >= nunits) {  /* big enough */
			if (p->s.size == nunits)  /* exactly */
                ...
                /*fprintf(stdout,"Tamaño libre antes %ld\n",p->s.size);*/
				p->s.size -= nunits;
                /*fprintf(stdout,"tamaño libre despues %ld\n",p->s.size);*/
				p+= p->s.size;
				p->s.size = nunits;
			}
            fprintf(stdout,"Lo estoy colocando en el nodo %d\n",i);
...
void xfree(void *ap)
{
	...
	*/
	...
	if (p + p->s.size == bp) {         /* join to lower nbr */
		...
		p->s.ptr = bp;
    /*fprintf(stdout,"Free size: %ld tamaño siguiente:%ld\n",p->s.size,p->s.ptr->s.size);*/
