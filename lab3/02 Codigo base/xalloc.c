...
static Header *morecore(size_t nu)
{
	...
	//printf("cp ini= %p cp fin= %p\n",sbrk(0),cp); //Hace un salto de nu veces el sizeOf
...
void *xmalloc (size_t nbytes)
{
	...
	*/
	...
	//printf("nunits: %ld sizeofHeader: %ld\n",nunits,sizeof(Header));
...
void xfree(void *ap)
{
	...
	bp = (Header *)ap - 1;  /* point to block header //printf("bp = (Header*) ap - 1 --> %p\n",bp);
	/*
	...
	*/
	...
	//	printf("freep --> p: %p p->s.prt %p  bp %p\n",p,p->s.ptr,bp);
		if (p >= p->s.ptr && (bp > p || bp < p->s.ptr))
			break;  /* freed block at start or end of arena */ 
	}

	/* Comprueba compactacion con hueco posterior */
	//printf("Hueco posterior --> bp %p bp->s.size %ld p->s.ptr %p\n",bp,bp->s.size,p->s.ptr);
	...
	//printf("Hueco anterior -->  p %p p->s.size %ld bp %p\n",p, bp->s.size,p->s.ptr);
