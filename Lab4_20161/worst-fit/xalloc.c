...
void *xmalloc (size_t nbytes)
{
	...
	if (worst != NULL){
		if (worst->s.size == nunits || worst->s.size == (nunits + 1)){ // Si es exacto o queda espacio de uno no genera hueco
			printf("Worst tiene espacio exacto\n");
			p->s.ptr = worst->s.ptr;
		} else {
			printf("Worst es mas grande que el espacio requerido\n");
			printf("Worst: %x - size: %d - siguiente: %x\n", worst, worst->s.size, worst->s.ptr);
			printf("Units del nuevo hueco: %d\n", worst->s.size - nunits);
			worst->s.size -= nunits;
			Align* p2 = (Align*)worst; // Esta parte es importante, como p es un puntero de Head y size esta en unidades de Align, casteamos el puntero como Align para poder sumarle el size.
		    p2 += worst->s.size;
...
void xfree(void *ap)
{
	...
	// Como la estrategia es worst-fit no se asigna a freep el hueco actual
