...
void simulate() {
  ...
  while (fscanf(fin, "%d, %c, %x", &pid, &ch, &vaddr) !=EOF) {
    ...
    //for LFU and MFU , "chance" being modified for the Second chance algorithm
    else{	
    ...
    printf("Marco referenciado: %x\n", pte->pfn);
   
    pte->reference = random()%2;
    pte->used = random()%2;
    printf("Reference: %d\n", pte->reference);
    printf("Used: %d\n", pte->used);
    pte->counter = ref_counter++; //used by LRU
    
    //Si se escribio en la pagina
...
/* make ./vmsim "second" example_trace.txt -p 5 -s 256*/